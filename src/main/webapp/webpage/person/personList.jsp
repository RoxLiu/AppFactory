...
<t:datagrid name="personList" title="" actionUrl="personController.do?datagrid&connectId=${connectId}" fit="true" fitColumns="true" idField="id" onClick="showProductList">
	<t:dgCol title="编号" field="id" hidden="false"/>
	<t:dgCol title="名字" sortable="false" field="name" query="false"/>
    <t:dgCol title="公司" field="company" query="false"/>
    <t:dgCol title="学校" field="secondTitle" query="false"/>
	<t:dgCol title="简介" field="brief" query="false"/>
    <t:dgCol title="电话" field="phone" query="false"/>
    ...
    <t:dgToolBar title="添加" icon="icon-add" url="personController.do?addorupdate&connectId=${connectId}" funname="add"/>
    <t:dgToolBar title="修改" icon="icon-edit" url="personController.do?addorupdate" funname="update"/>
</t:datagrid>
</div>
</div>
<div region="east" style="width: 500px;" split="true">
<div class="easyui-panel" title="个人商品" style="padding: 0px;" fit="true" border="false" id="product-panel"></div>
</div>
